Class('BryntumGridTest', {
    ...
    override : {
        ...
        setup : function(callback, errback) {
            ...
            if (this.global.bryntum && this.global.bryntum.grid) {
                ...
                for (let className in this.global.bryntum.grid) {
            ...
            if (!harness.getDescriptorConfig(harness.getScriptDescriptor(me), 'usesConsole')) {
                ...
                me.global.console.error =
                    me.global.console.warn =
                        me.global.console.log =
                            me.global.console.info = function(msg) {
    ...
    methods : {
        ...
        performPostTestSanityChecks : function(evt, t) {
        ...
        touchStart : function(element, identifier) {
            if (!identifier) {
        ...
        checkGridSanity : function(grid) {
            var columns = grid.columns.leaves.filter(col => !col.hidden),
                shrinkwrapGroupColumns = grid.columns.allRecords.filter(c => c.children && !c.width && !c.flex && c.children.every(cc => cc.width)),
            ...
            for (i = 0; i < shrinkwrapGroupColumns.length; i++) {
                const el = shrinkwrapGroupColumns[i].element,
            ...
            for (i = 0; i < rowCount; i++) {
                ...
                for (j = 0; j < columnCount; j++) {
                    cell = grid.getCell({ id : row.id, columnId : columns[j].id });
                    if (cell.offsetWidth !== headerEls[j].offsetWidth) {
                        this.fail(`Grid ${grid.id}, cell[${i}].offsetWidth (${cell.offsetWidth}) != header[${columns[j].id}].offsetWidth (${headerEls[j].offsetWidth}})`);
                    }
                }
            }
        },
        ...
        getDSTDates : function(year = 2012) {
            ...
            for (let i = yearStart; i <= yearEnd; i = new Date(i.setDate(i.getDate() + 1))) {
                let midnightOffset = new Date(year, i.getMonth(), i.getDate()).getTimezoneOffset(),
        ...
        assertNoDomGarbage : function() {
            ...
            this.contentNotLike(body, /L{.*?}/, 'No non-localized string templates L{xx}');

            // do not run this check in framework demos, or renderers demos since it test code editor which has a valid "undefined" in it
        ...
        assertNoResizeMonitors : function() {
        ...
        waitForRowsVisible : function(grid, next) {
        ...
        waitForAnimationFrame : function(next) {
        ...
        mockUrl : function(url, response) {
            ...
            // Inject the overrides into the AjaxHelper instance
        ...
        mockAjaxGet : function(url, options) {
        ...
        mockAjaxPost : function(url, data, options) {
            var result = this.mockAjaxMap[url.split('?')[0]];
            var win    = this.global;
        ...
        getFirstRenderedRow : function(grid) {
            var root = grid && grid.element || this.global.document.body;
        ...
        getLastRenderedRow : function(grid) {
            var root = grid && grid.element || this.global.document.body;
            var rows = root.querySelectorAll('.b-grid-row');
            var y    = -1;
            var row;
        ...
        isRectApproxEqual : function(rect1, rect2, threshold, descr) {
            for (var o in rect1) {
                this.isApprox(rect1[o], rect2[o], threshold, descr || '');
            }
        },

        startVideoRecording : function(callback) {
            const me       = this,
                appIds   = {
