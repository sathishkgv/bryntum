import { BrowserHelper } from '../../build/gantt.module.js?433978';
...
StartTest(t => {
    ...
    t.it('Should render and update dependencies (w/o provided project)', t => {
        ...
        t.chain(
            { drag : '.b-gantt-task.id11', by : [gantt.tickWidth, 0] },
    ...
    // FIXME: the test seems outdated and cannot rely on "dependenciesdrawn" event anymore
    // ..fails for umd & module bundles in all browsers except chrome
    t.snooze('2019-07-30'/* 'Should render dependencies effectively on event change' */, t => {
        gantt = t.getGantt({
            appendTo : document.body,
            columns  : [
                { type : 'name' }
        ...
        t.chain(
            ...
            next => {
                t.waitForEvent(gantt, 'dependenciesdrawn', next);
                event.setConstraintDate(new Date(2017, 0, 16));
            }
        );
    });
    ...
    t.it('Should render dependencies effectively on dependency change', t => {
        ...
        t.chain(
            { waitForPropagate : gantt },
            next => t.waitForEvent(gantt, 'dependenciesdrawn', next),
            next => {
                t.isCalledNTimes('drawDependency', gantt.features.dependencies, 3);
                t.isCalledNTimes('findPath', gantt.features.dependencies.pathFinder, 3);
    ...
    t.it('Should render dependencies to milestones outside of the view', t => {
        ...
        t.chain(
            ...
            next => {
                ...
                t.isGreater(lineEnd, gantt.timeAxisViewModel.totalWidth, 'Last dependency vertical line is hidden');
