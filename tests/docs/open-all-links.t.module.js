...
describe('Open all links in docs tree and assert correct conten + no crashes', t => {
                    ...
                    (next) => {
                        ...
                        Array.from(contentElement.querySelectorAll('a[href^="#"]:not(.summary-icon):not(.inherited)')).forEach((node) => {
                            ...
                            if (!clsRecord) {
                                ...
                                if (parts.length > 1) {
                                    found = (clsRecord.data[propertyName] || []).find(mem => {
    ...
    t.chain(
        ...
        next => {
            ...
            store.getById('Gantt').traverse(classRecord => {
