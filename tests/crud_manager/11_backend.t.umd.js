...
StartTest(function (t) {
  var calendarManager, resourceStore, assignmentStore, dependencyStore, taskStore; // get test descriptor (it has backend sync/load/reset URLs used in the test)
  ...
  t.beforeEach(function (t, next) {
    ...
    AjaxHelper.get(testConfig.resetUrl).then(next).catch(function () {
  ...
  t.it('Should be possible to save some resources, assignments, dependencies and tasks', function (t) {
    ...
    regeneratorRuntime.mark(function _callee6() {
      ...
      action: function action(next) {
        AjaxHelper.get(testConfig.setupUrl).then(next).catch(function () {
          return t.fail('DB restoring failed');
        });
      },
