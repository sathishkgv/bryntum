import { AjaxHelper, TimeUnit, SchedulingMode, ConstraintType } from '../../build/gantt.module.js?433978';
...
StartTest(function(t) {
    ...
    const task   = name => findByName(taskStore, name),
    ...
    t.beforeEach((t, next) => {
        ...
        project = new ProjectModel({
            ...
            transport : {
                load : Object.assign({ method : 'GET', paramName : 'q' }, testConfig.load),
                sync : Object.assign({ method : 'POST' }, testConfig.sync)
            },
            listeners : {
        ...
        AjaxHelper.get(testConfig.resetUrl)
            .then(next)
    ...
    t.it('Should be possible to save some resources, assignments, dependencies and tasks', function(t) {
        ...
        t.chain(
            ...
            {
                action : next => {
                    AjaxHelper.get(testConfig.setupUrl)
                        .then(next)
                        .catch(() => t.fail('DB restoring failed'));
                },
                desc : 'Restoring PHP demo database'
            }
        );
    });
/*
