...
StartTest(t => {
    ...
    t.it('Validating dependencies while D&D', t => {
        ...
        const oldUpdateTipContent = gantt.features.dependencies.updateTipContent;

        gantt.features.dependencies.updateTipContent = (tip, data) => {
            if (data.valid instanceof Promise) {
                const origPromise = data.valid;

                data.valid = new Promise(resolve => {
                    origPromise.then(valid => {
                        setTimeout(() => {
                            resolve(valid);
                        }, 500);
                    });
