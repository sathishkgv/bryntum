import { AjaxHelper, DomHelper, EventHelper, WidgetHelper, Scroller, Popup, GlobalEvents, VersionHelper, StringHelper } from '../../../build/gantt.module.js?433978';
...
class ExamplesApp {
    constructor() {
        ...
        me.isOnline = /bryntum.com|^lh$/.test(location.host);
    ...
    template(data) {
        ...
        return data.examples.map(example => {
            ...
            return `${example.firstInGroup ? `
                ...
                ` : ''}
                <a
                    class="example ${isNew(example) ? 'new' : ''}"
                    id="${example.folder}"
                    href="${example.folder}"
    ...
    render() {
        ...
        EventHelper.on({
            element : examplesThumbsContainerEl,
            click(event) {
                ...
                promises = me.examples.map(example => {
                    ...
                    return AjaxHelper.get(example.folder + '/app.config.json', { parseJson : true }).then(response => {
                        if (response.parsedJson) {
                            example.tooltip = response.parsedJson.description.replace(/[\n\r]/g, '').replace(/"/g, '\'');
                        }
                    });
                });
