import { WidgetHelper, AjaxHelper, DomHelper, IdHelper, Tooltip, Toast, BrowserHelper, StringHelper, LocaleManager, Localizable, DataGenerator, Fullscreen, Events, GlobalEvents, VersionHelper, EventHelper, Panel, ResizeHelper, Rectangle } from '../../build/gantt.module.js?433978';
...
const
    ...
    isModule = pathName.endsWith('module.html') || (moduleTag && moduleTag.src.includes('app.module.js?433978')) || (pathName.endsWith('index.html') && isOnline),
...
class Shared extends Localizable(Events()) {
            ...
            async onToggle({ pressed }) {
                if (pressed) {
                    if (!shared.codeEditor) {
                        shared.codeEditor = await CodeEditor.addToPage();
                    }
    ...
    initRootCause() {
        const
            isExcluded = location.href.includes('csp') || isDemoBrowser,
...
class CodeEditor extends Panel {
    ...
    static get defaultConfig() {
        return {
            ...
            keep : true, // To not be removed on successfull import
            ...
            tbar : [
                {
                    ...
                    ref      : 'filesCombo',
                    editable : false,
                    value    : isModule ? 'app.module.js?433978' : isUmd ? 'app.umd.js?433978' : 'app.js?433978',
                    items    : [
                        isModule ? 'app.module.js?433978' : isUmd ? 'app.umd.js?433978' : 'app.js?433978'
                    ],
    ...
    animate() {
        ...
        this.setTimeout(() => this.element.classList.remove('animating'), 200);
    }
    ...
    async updateJS() {
        const
            me = this,
            code = me.pre.innerText + '\nexport default null;\n';

        me.codeCache[me.filename] = me.pre.innerText;

        let renderedWidgets = new Set();

        // Store all current widgets to be able to cleanup on import fail. If the import fails because of a
        ...
        try {
            ...
            if (response.parsedJson.success) {
                ...
                // Destroy pre-existing grids etc. after the import, to lessen flickering
                renderedWidgets.forEach(widget => !widget.keep && widget.destroy());
    ...
    async loadCode(filename = isModule ? 'app.module.js?433978' : isUmd ? 'app.umd.js?433978' : 'app.js?433978') {
        const me = this;
        ...
        if (filename.endsWith('.js?433978')) {
            me.mode = 'js';
            code = me.processJS(code);
        }
        else if (filename.endsWith('.css?433978')) {
            me.mode = 'css';
            code = me.processCSS(code);
        }

        me.html = `<div class="scroller"><pre><code>${code}</code></pre></div><div class="status">${exception ? exception.message : 'Idle'}</div>`;
    ...
    updateDownloadLink() {
        ...
        downloadLink.href = `data:text/${this.filename.endsWith('.css?433978') ? 'css' : 'javascript'};charset=utf-8,${escape(this.pre.innerText)}`;
    }
    ...
    static async addToPage() {
        ...
        if (document.head.querySelector('[href$="app.css?433978"]')) {
            filesStore.add({ text : 'resources/app.css?433978', value : 'resources/app.css?433978' });
        }
