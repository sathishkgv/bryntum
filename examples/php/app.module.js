import { Toolbar, Toast, DateHelper, IdHelper, CSSHelper, PercentDoneColumn, ColumnStore, ResourceAssignmentColumn, Column, TaskModel, Gantt, Panel, ProjectModel, EffectResolutionResult } from '../../build/gantt.module.js?433978';
...
const project = window.project = new ProjectModel({
    ...
    listeners : {
        'loadfail' : ({ response }) => {
            // data loading failed ..and response text was not decoded
            if (!response) {
                Toast.show({
                    html : 'Could not load the data from the server.<br>' +
                        '<b>Please make sure that you\'ve read readme.md file carefully and <br>' +
                        'setup the database connection accordingly.</b>',
                    timeout : 30000
                });
            }
        }
...
project.load().then(() => {
    ...
    panel.tbar.widgetMap.startDateField.on({
        change : 'up.onStartDateChange'
    });
});
