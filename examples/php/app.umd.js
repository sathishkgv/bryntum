...
function (_bryntum$gantt$Toolba) {
  ...
  _createClass(GanttToolbar, [{
    ...
    value: function () {
      ...
      return function onAddTaskClick() {
...
var project = window.project = new bryntum.gantt.ProjectModel({
  ...
  listeners: {
    'loadfail': function loadfail(_ref14) {
      var response = _ref14.response;

      // data loading failed ..and response text was not decoded
      if (!response) {
        bryntum.gantt.Toast.show({
          html: 'Could not load the data from the server.<br>' + '<b>Please make sure that you\'ve read readme.md file carefully and <br>' + 'setup the database connection accordingly.</b>',
          timeout: 30000
        });
      }
    }
