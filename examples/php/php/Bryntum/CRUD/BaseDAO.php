...
class BaseDAO
{
    ...
    public function __construct($dsn, $dbuser, $dbpwd, $dboptions = null)
    {
        try {
            ...
            throw new Exception('Database connection error.', E_APP_DATABASE_CONNECTION);
        }
    }
    ...
    public function getFoundRows()
    {
        if (!$r = $this->db->query('select found_rows()')) {
            throw new Exception('Cannot get total number of rows.', E_APP_FOUND_ROWS);
        }
    ...
    public function getTableCols($table)
    {
        ...
        if (!$r = $this->db->query('show columns from '.$table)) {
            throw new Exception('Cannot get columns of '.$table, E_APP_SHOW_COLUMNS);
        }
    ...
    public function getOption($name)
    {
        if (!$r = $this->db->query('select value from options where name = \''.$name.'\' limit 1')) {
            throw new Exception('Cannot get option '.$name.'.', E_APP_GET_OPTION);
        }
    ...
    public function updateRevision()
    {
        if (!$this->db->exec('update options set value = value + 1 where name = \'revision\'')) {
            throw new Exception('Cannot update server revision stamp.', E_APP_UPDATE_REVISION);
        }
